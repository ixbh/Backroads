...
- `load_scenic_features.py` and `scenery.py`: import explicit OSM landscape and sightseeing features. Foliage-based scenery scoring remains separate from optional parks, viewpoints, attractions, and monuments.
- `routing.py`: builds directed NetworkX graphs from exact OSM node identities, preserves one-way restrictions, filters known unpaved surfaces, keeps residential roads out of the preferred graph, avoids dead-end waypoint anchors and exact out-and-back retraces, and uses preference-aware anchors so curviness/scenery affect the loop shape as well as each path.
- `api.py`: FastAPI adapter for route generation, address lookup, and response summaries, with a bounded five-minute cache for the most recent expensive graph build.
- `geocoding.py`: replaceable server-side Nominatim adapter with explicit-search-only requests, a bounded cache, and a one-request-per-second throttle.
...
3. Apply `schema.sql`, import an OSM extract with `load_osm.py`, and run `curviness.py`. Existing databases created before the topology update should apply `migration_road_topology.sql`; existing scenery databases should also apply `migration_attractions.sql`.
...
- `GEOCODER_BASE_URL`, `GEOCODER_USER_AGENT`, and `GEOCODER_COUNTRY_CODES` — server-side address-provider settings. The UI never receives a provider key.
...
- Nominatim — default address-search provider. Searches happen only after the user presses the address button, are cached and globally throttled, and can be redirected to another compatible provider through configuration.
...
- Sightseeing mode biases roads with nearby OSM parks, viewpoints, attractions, or monuments; it does not yet insert a named stop or guarantee venue access/opening hours.
- No toll, motorway, café/fuel-stop, or live traffic preferences yet.
